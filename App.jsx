...
const PRESETS = [
  {
    id: "theta", name: "Тета-поле", subtitle: "4–7 Гц · слайд Зеланда",
    description: "Межа між свідомим і підсвідомим. Ідеально для прописування слайдів і констатації реальності.",
    beat: 6, carrier: 200, noise: "pink", noiseVolume: 0.03, color: "#7C3AED", glow: "#A78BFA",
  },
  {
    id: "schumann", name: "Резонанс Шумана", subtitle: "7.83 Гц · частота Землі",
    description: "Природна частота електромагнітного поля Землі. Синхронізує нервову систему з планетарним фоном.",
    beat: 7.83, carrier: 180, noise: "brown", noiseVolume: 0.04, color: "#065F46", glow: "#34D399",
  },
  {
    id: "alpha", name: "Альфа-намір", subtitle: "10 Гц · спокійна присутність",
    ...
    beat: 10, carrier: 220, noise: "pink", noiseVolume: 0.025, color: "#1D4ED8", glow: "#60A5FA",
  },
];
...
export default function ZelandSound() {
  ...
  const nextQuote = () => {
    setQuoteFade(false);
    setTimeout(() => {
      setQuoteIndex(i => { let n=i; while(n===i) n=Math.floor(Math.random()*QUOTES.length); return n; });
      setQuoteFade(true);
    }, 300);
  };
      ...
      setElapsed(prev => {
        ...
        if (next >= durationRef.current * 60) {
          ...
          setTimeout(() => {
            ...
            if (audioCtxRef.current) { try { audioCtxRef.current.close(); } catch {} audioCtxRef.current = null; }
            masterGainRef.current = null;
            setIsPlaying(false); setElapsed(0); setFinished(false);
          }, 5000);
          return next;
        }
        return next;
  ...
  const handlePreset = (preset) => {
    if (isPlaying && activePreset?.id === preset.id) {
      stopAudio(false); setActivePreset(null);
    } else {
      setActivePreset(preset); startAudio(preset);
    }
  };
        ...
        <div className="left-col">
          ...
          <div className="presets">
            {PRESETS.map(preset => {
              ...
              return (
                <button key={preset.id} onClick={() => handlePreset(preset)} className="preset-btn" style={{
                  ...
                  {active && <div style={{position:"absolute",inset:0,background:`radial-gradient(ellipse at 15% 50%,${preset.glow}08 0%,transparent 65%)`,pointerEvents:"none"}}/>}
                  <div className="preset-inner">
                    <div className="preset-text">
                      <div className="preset-name-row">
                        <span style={{fontSize:"15px",fontWeight:"500",color:active?preset.glow:"#cbd5e1"}}>{preset.name}</span>
                        <span className="preset-tag" style={{color:active?preset.glow:"#475569",background:active?`${preset.color}22`:"rgba(255,255,255,0.05)",border:`1px solid ${active?preset.color+"44":"transparent"}`}}>{preset.subtitle}</span>
                      </div>
                      <p className="preset-desc">{preset.description}</p>
                    </div>
                    <div className="preset-icon" style={{border:`1.5px solid ${active?preset.glow:"#334155"}`,background:active?`${preset.color}33`:"transparent"}}>
                      {active ? (
                        <div style={{display:"flex",gap:"2px",alignItems:"center"}}>
                          {[0,1,2].map(i=><div key={i} style={{width:"2px",background:preset.glow,borderRadius:"2px",animation:`wave 0.8s ease-in-out ${i*0.15}s infinite alternate`,height:`${8+i*3}px`}}/>)}
                        </div>
                      ) : (
        ...
        <div className="right-col">
          <div className="ring-wrap">
            <svg width="180" height="180" style={{transform:"rotate(-90deg)"}}>
              ...
              <circle cx="90" cy="90" r={R} fill="none" stroke={finished?"#34D399":ringColor} strokeWidth="7" strokeLinecap="round" strokeDasharray={CIRC} strokeDashoffset={dash} style={{transition:"stroke-dashoffset 1s linear,stroke 0.5s"}}/>
            </svg>
            <div className="ring-center">
              {finished ? <div style={{fontSize:"36px",animation:"bellPulse 1s ease-in-out 3"}}>🔔</div>
                : isPlaying ? <><span className="ring-time">{fmt(remaining)}</span><span className="ring-label">залишилось</span></>
                : <><span className="ring-idle-num">{duration}</span><span className="ring-idle-label">хвилин</span></>}
            </div>
          </div>
          ...
          <div className="controls-card">
            <div className="control-row">
              ...
              <input type="range" min="0" max="1" step="0.01" value={volume} onChange={e=>setVolume(parseFloat(e.target.value))} style={{width:"100%",accentColor:activePreset?.color||"#7C3AED"}}/>
            </div>
            <div className="control-row" style={{marginBottom:0}}>
              ...
              <div className="duration-btns">
                {[5,10,15,20,30].map(m=>(
                  <button key={m} onClick={()=>{setDuration(m);durationRef.current=m;if(isPlaying)setElapsed(0);}} className="dur-btn" style={{
          ...
          <p className="tip">🎧 Навушники обов'язкові<br/><em>Констатуй, не благай — дзвін сповістить про кінець</em></p>
        </div>
      </div>

      <style>{`
        *{box-sizing:border-box}
        .root{min-height:100vh;background:linear-gradient(135deg,#0a0a0f 0%,#0d0d1a 50%,#0a0f0a 100%);display:flex;align-items:center;justify-content:center;font-family:'Inter',system-ui,sans-serif;color:#e2e8f0;padding:40px 24px}
        .layout{width:100%;max-width:900px;display:grid;grid-template-columns:1fr 320px;gap:40px;align-items:start}
        ...
        .preset-btn{position:relative;overflow:hidden;border-radius:16px;padding:18px 20px;cursor:pointer;text-align:left;transition:all 0.3s ease}
        ...
        .preset-icon{width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin-left:16px;flex-shrink:0;transition:all 0.3s}
        .quote-block{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:16px;padding:22px 24px;transition:opacity 0.3s ease}
        ...
        .controls-card{width:100%;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:16px;padding:20px 22px}
